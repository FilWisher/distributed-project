...
def unPickle(path):
    ...
    '''
    resFile = pickle.load(open(path))
    bulkData = resFile[0][1]['BULK_DATA']['ALL_SOURCES']
    keys = sorted(bulkData.keys())
    start = bulkData[keys[0]]
    print "start: " + str(start)

    f = open("oneCacheLocations", "w+")

    for a in keys:
        if bulkData[a] != start:
            #print a
            print (bulkData[a], a)
            f.write(str((bulkData[a],a)))
            start = bulkData[a]
...
def pullTopology():
    ...
    topObj = icarus.registry.TOPOLOGY_FACTORY['SIX_NODE']()
